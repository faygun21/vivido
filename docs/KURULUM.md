# Kurulum ve Geliştirici Rehberi

> Bu dosya, projeyi **kendi makinesinde çalıştıracak** geliştiriciler içindir.
> Projenin ne olduğunu merak ediyorsanız kök dizindeki
> [README](../README.tr.md) tanıtım sayfasına bakın.
>
> Bu içerik eskiden `README.md` idi; depo herkese açılırken kök README bir
> tanıtım sayfasına dönüştüğü için buraya taşındı (2026-09-07).
...
📖 Tam teknik tasarım: [`01-PROJE-PLANI.md`](01-PROJE-PLANI.md)
🧭 Uygulama sırasında verilen kararlar ve gerekçeleri: [`02-KARARLAR.md`](02-KARARLAR.md)
...
> üretiyordu ([K-12](02-KARARLAR.md#k-12)).

**Doğrulama:** `curl http://localhost:5000/health/ready` → `Healthy`

Konut/POI verisini de istiyorsan: [`04-MEVCUT-DURUM.md §3`](04-MEVCUT-DURUM.md).
...
Mobil **Flutter** ile yazılıyor ([K-08](02-KARARLAR.md#k-08)) — Expo/React
...
[`02-KARARLAR.md` K-09](02-KARARLAR.md#k-09).
...
Doğrulama adımını geçici olarak kapatmak istersen — **varsayılan AÇIK**,
yukarıdaki kutuya bak — `.env`:
...
([K-09 gerekçe §4](02-KARARLAR.md#k-09)).

### 2.6 Admin panelini açmak — ilk admin

Panel (`/admin`) `AdminOnly` yetkisi ister ve "admin yap" ucu da admin
olmayı gerektirir: **kısır döngüyü kıran tek şey `BootstrapAdmin:Email`**.
Bu e-postayla **giriş yapan** kullanıcıya otomatik admin yetkisi verilir
(`BootstrapAdminService`). Kullanıcının önce kayıt olup e-postasını
doğrulamış olması gerekir.

> ⛔ **Bu adresi `appsettings*.json` içine YAZMA** — o dosyalar git'e giriyor
> ve yazılan adrese sahip **herkes** ilk girişinde admin olur. Adres ileride
> el değiştirirse (kurum/üniversite hesapları geri dönüşümlü olabiliyor)
> yetki de onunla gider.

**A) `pnpm dev:api` ile çalıştırıyorsan → user-secrets:**

```powershell
dotnet user-secrets --project api/src/Vivido.Api set "BootstrapAdmin:Email" "senin.adresin@ornek.com"
```

**B) `docker compose` ile çalıştırıyorsan → `.env`:**

```dotenv
BootstrapAdmin__Email=senin.adresin@ornek.com
```

Sunucuda karşılığı `deploy/.env` içindeki aynı satır. Boş bırakılırsa panel
hiç kullanılamaz. Yetki verildiğinde API loguna
`Bootstrap admin yetkisi verildi: <UserId>` satırı düşer.
