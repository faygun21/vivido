...
export function GuideMascot({ 
  ...
  const dynamicSteps = steps.map((step, index) => {
    if (index === 6) {
      return {
        ...step,
        text: isGuest
          ? 'Son olarak, ücretsiz hesap oluşturduğunda sağ üst köşede "En Uygun Evler" butonu belirecek. Bu butona tıklayarak sadece sana özel hesaplanmış ev listesini, uyum puanlarını ve detaylarını görebilirsin.'
  ...
  }, [currentStepIndex, isTourActive, isOpen, isBubbleVisible, isGuest, steps, onTabChange]);
  ...
  return (
    <div
      style={{
        position: 'absolute',
        bottom: isWideScreen ? '20px' : 'calc(45vh - 7px)',
        top: 'auto',
        left: isOpen ? (isWideScreen ? '343px' : '20px') : '-200px',
        right: 'auto', 
    ...
    >
      <img
        src={isWideScreen ? "/mascot_on_the_wall.png" : "/mascot_2.png"}
        alt="Rehber Maskot"
        style={{
          width: isWideScreen ? '50px' : '60px', 
          height: 'auto',
          objectFit: 'contain',
     ...
     {isBubbleVisible && (
        <div
          style={{
            ...
            position: 'absolute',
            ...(isWideScreen 
              ? { bottom: '100%', left: '10px', marginBottom: '10px' } 
              : { bottom: '5px', left: '75px' } 
            ),
            width: '230px',
            pointerEvents: 'auto',
